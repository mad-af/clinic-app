        ...
        return $form
            ->schema([
                ...
                Forms\Components\Select::make('patient_visit_id')
                    ->label('Patient Visit (Optional)')
                    ->options(function (Forms\Get $get) {
                        $patientId = $get('patient_id');
                        if (! $patientId) {
                            return [];
                        }
                        return \App\Models\PatientVisit::where('patient_id', $patientId)
                            ->orderBy('created_at', 'desc')
                            ->get()
                            ->mapWithKeys(function ($visit) {
                                return [$visit->id => $visit->created_at->format('d M Y H:i') . ' - ' . $visit->doctor->name];
                            });
                    })
                    ->searchable()
                    ->preload()
                    ->visible(fn (Forms\Get $get) => filled($get('patient_id'))),
